...
public:
	Wagon(const std::string& payload,int number)
		: Payload(payload)
	{
		this->number = number;
		//printf("%s\n", __FUNCTION__);
	...
	/*
	bool operator<(const Wagon::Ptr& other) {
		return Payload < other->GetPayload();
	}
	*/

};
inline bool operator< (const Wagon::Ptr& lhs, const Wagon::Ptr& rhs) {
